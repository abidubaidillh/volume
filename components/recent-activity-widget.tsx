...
export function RecentActivityWidget({ userId }: RecentActivityWidgetProps) {
  ...
  const getActivityText = (activity: RecentActivity) => {
    switch (activity.type) {
      ...
      case 'wishlist':
        return `Added "${activity.book_title}" to wishlist`
      case 'reading_list':
        return `Added "${activity.book_title}" to reading list`
      default:
        return `Activity on "${activity.book_title}"`
    }
  }
    ...
    >
      {!loading && activities.length === 0 ? (
        <div className="text-center py-8 text-zinc-500">
          ...
          <p className="text-xs">Start by searching for a book!</p>
        </div>
      ) : (
